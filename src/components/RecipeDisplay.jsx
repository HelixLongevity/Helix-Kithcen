...
/** Build a plain-text version of the recipe for sharing */
function buildShareText(recipe) {
  const lines = []
  const components = recipe.meal_components || [{ component_name: null, ingredients: recipe.ingredients || [], steps: recipe.steps || [] }]

  lines.push(`🍽️ ${recipe.title}`)
  lines.push(`From Helix Kitchen — kitchen.helixlongevity.com.au`)
  lines.push('')

  if (recipe.description) lines.push(recipe.description, '')

  const timeParts = []
  if (recipe.prep_time_minutes) timeParts.push(`Prep ${recipe.prep_time_minutes} min`)
  if (recipe.cooking_time_minutes) timeParts.push(`Cook ${recipe.cooking_time_minutes} min`)
  if (recipe.difficulty) timeParts.push(recipe.difficulty.charAt(0).toUpperCase() + recipe.difficulty.slice(1))
  if (timeParts.length) lines.push(`⏱️ ${timeParts.join(' · ')}`, '')

  if (recipe.servings) lines.push(`Serves ${recipe.servings}`, '')

  for (const comp of components) {
    if (comp.component_name) lines.push(`── ${comp.component_name} ──`, '')
    ...
    if (comp.steps?.length) {
      ...
      comp.steps.forEach((step, i) => {
        const prefix = step.title ? `${i + 1}. ${step.title}: ` : `${i + 1}. `
  ...
  if (recipe.tips?.length) {
    lines.push("CHEF MARCO'S TIPS")
    recipe.tips.forEach(tip => lines.push(`★ ${tip}`))
    lines.push('')
  }

  lines.push('Generated by Helix Kitchen · kitchen.helixlongevity.com.au')
  return lines.join('\n')
}
...
export default function RecipeDisplay({ recipe, onNewRecipe, isFavourite, onToggleFavourite, onPrint, onNutritionData, onCacheNutrition }) {
  ...
  const [shareToast, setShareToast] = useState(null) // 'shared' | 'copied' | 'error'

  const handleShare = useCallback(async () => {
    const text = buildShareText(recipe)
    const shareData = { title: recipe.title, text }
    ...
    if (navigator.share && navigator.canShare?.(shareData)) {
      try {
        ...
        if (err.name !== 'AbortError') setShareToast('error')
      }
    } else {
        ...
        <div className="hk-hero px-6 pt-8 pb-6">
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div className="flex items-center gap-2 mt-4 flex-wrap">
              ...
              <button
                onClick={handleShare}
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer border bg-navy/40"
                style={shareToast
                  ? { borderColor: 'rgba(200,149,42,0.5)', color: '#C8952A' }
                  : { borderColor: 'rgba(51,65,85,0.6)', color: '#cbd5e1' }
                }
                title="Share recipe"
              >
                {shareToast === 'copied' ? (
